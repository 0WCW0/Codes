...
<div class="container">
    ...
    <div class="row">
        ...
        <div align="center">
            ...
            <form action="searchU.do" method="post">
                搜索：<input type="input" id="searchU" name="searchU" placeholder="输入用户名" />
            ...
            <table id="data_list" class="table table-hover table-bordered" border="1" cellspacing="0" cellpadding="0" width="500px" style="margin:0 auto">
                <caption>user信息（<a href="insU.jsp">新增</a>）</caption>
                <tr>
                    <th>编号</th>
                    <th>用户名</th>
                    <th>密码</th>
                ...
                <!-- 使用c标签的遍历: 11种c标签; -->
                <c:forEach items="${upmList }" var="u" varStatus="i">
                    <tr class="row${i.index % 2 + 1 }">
                        ...
                        <td>${u.type }</td>
                        <td>
                            <a href="delU.do?id=${u.id }" onclick="return confirm('您是否确认删除${u.username }的信息?')">删除</a>
                            <a href="loadU.do?id=${u.id }" onclick="return confirm('您是否确认修改${u.username }的信息?')">修改</a>
                        </td>
                    </tr>
                </c:forEach>
            </table>
            ...
            <div class="page" style="width:350px; margin:0 auto;">
                <div class="page-information">
                    ...
                    <a href="${path}listU.do?pageNum=1" class='first'>首页</a>
                    <a href="${path}listU.do?pageNum=${page.pageNum>1?page.pageNum-1:1}" class='pre'>上一页</a>
                    <c:forEach begin="1" end="${page.pageTitle}" var="i">
                        <a href="${path}listU.do?pageNum=${i}" style="text-decoration: none;">${i}</a>
                    </c:forEach>
                    <a href="${path}listU.do?pageNum=${page.pageNum<page.pageTitle?page.pageNum+1:page.pageTitle}" class='next'>下一页</a>
                    <a href="${path}listU.do?pageNum=${page.pageTitle}" class='last'>末页</a>
                </div>
            </div>
